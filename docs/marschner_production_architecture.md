...
| --- | --- | --- | --- | --- |
| Fast | `hair_marschner_unity_fast.gdshader` | Unity Standard separable Gaussian | Unity preintegrated RGB 3D LUT | Unity Standard formula/parameterization |
...
Uniform and varying declarations intentionally remain in the top-level `.gdshader` wrappers. A previous Godot 4.7 issue showed that uniforms declared only through nested includes are not reliably exposed to runtime material binding. The body includes therefore contain implementation code, not the reflected material interface.
...
Fast validation should keep these concerns separate:

1. Unity source formula/parameterization parity;
2. Unity azimuthal LUT numerical validation;
3. finite/renderable output and production material binding;
4. baseline energy/angular/visual differences as reports;
5. GPU timing.
...
1. generates both production LUTs;
2. validates both LUTs numerically;
3. runs `test_marschner_production_profile.gd` to verify shader selection, top-level uniform reflection, LUT reconstruction/binding, Fast eta pinning, and Cinematic beta metadata;
4. reports complete Cinematic energy parity;
5. runs the existing windowed GPU comparison unless `--skip-runtime` is passed;
6. writes `benchmark/results/marschner_tier_split_study.json`.
