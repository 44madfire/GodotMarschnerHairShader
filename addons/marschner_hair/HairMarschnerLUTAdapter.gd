...
## The shader slot owns the semantic contract; this adapter verifies the texture
## dimensions, format, and rendering RID before binding it.
...
func bind_unity_fast(material: ShaderMaterial, data: Resource = null) -> bool:
	if material == null:
		return false
	var texture: Texture3D = load_default_unity_texture() if data == null else data as Texture3D
	if texture == null:
		return false
...
## Compatibility hook retained for callers written against the former cached adapter.
